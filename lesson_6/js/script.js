let open = document.getElementById('open-btn'),
    ...
    price_value = document.querySelector('.price-value'),
    discount_check = document.querySelector('.discount_check');

console.log(discount_check.value);
...
employers_btn.addEventListener('click', () => {
  for (let i = 0; i < hire_employers_item.length; i++) {
			let employersName = hire_employers_item[i].value;
...
discount_check.addEventListener('change', () => {
   if (discount_check.checked == true) {
  	discount_value.style.backgroundColor = 'green';
  	price_value.value = price_value.value * 0.8;
 } else {
